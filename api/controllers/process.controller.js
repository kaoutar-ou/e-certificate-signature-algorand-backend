    ...
    if (students.length > 0 && signers.length > 0 && titre_diplome !== "" && template !== "" && date !== "" && local !== "" && filiere !== "") {
        Promise.all(students.map(async (student) => {
            ...
            const data_ = {
                general: {
                    image: template,
                    ministere: ministere ? ministere : null,
                    presidence: presidence ? presidence : null,
                    etablissement: etablissement ? etablissement : null,
                    fileName: filename,
                    template: template
                },
                test: {
            ...
            const responseBody = await axios.post('http://localhost:7003/api/process/generate-certification', {data: data_, student});
...
//     // }
//     try {
//         const response = await axios.post('http://localhost:7003/api/backops/upload-file', formData);
...
//             const fileNameDist = await axios.post('http://localhost:7003/api/backops/upload-file', {data: data_});
...
//                 fileName: encryptFilename(filename),
//             });
//             console.log("🚀 ~ file: process.controller.js ~ line 230 ~ generateCertificat ~ certificat", certificat)


//             await certificat.setEtudiant(student_.dataValues.id);
//             await certificat.setFiliere(filiere_.dataValues.id);

//             console.log("🚀 ~ file: process.controller.js ~ line 236 ~ generateCertificat ~ certificat", certificat)

//             await student_.addCertificat(certificat);

//             console.log("🚀 ~ file: process.controller.js ~ line 239 ~ generateCertificat ~ student_", student_)
...
//     console.log("req--------------------------------------");
//     try {
//         const response = await axios.get("http://localhost:7003/api/process/get-certificate?hash="+req.query.hash);
